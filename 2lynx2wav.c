...
//	  	  	    value=-100 * sin(x*val);
//	  	  	    value=(value*boost)+128;			  
//	  	  	    if (value > 127) value = 255;
//	  	  	    else value = 0;
			  
		     
			   //if ((i > 0) && (i <= (size-1)/2)) value = 0;
			   //else value = 255;
			   if (i <=	  (size-1)/2) value = 0;
			   else value = 255; 
...
int init(int argc, char *argv[])
{
	...
	for (i = 1; i < argc - 2; i++)
	{
		...
		else if (strcmp(argv[i], "-f22") == 0) {
		   	speed = 22050;  			 	    	        	   	    
		}
		else if (strcmp(argv[i], "-f44") == 0) {
		   	speed = 44100;  			 	    	        	   	    
		}	   	   	     	   
		else if (strcmp(argv[i], "-r6") == 0) {
			tape_1 = 4;
			square = 1;
		   	speed = 2*3150;	    		
		}	   	   	      	          	    	   
		else if (strcmp(argv[i], "-r9") == 0) {
			tape_1 = 6;
			square = 1;
		   	speed = 3*3150;	    		
		}	   	   
...
int main(int argc, char *argv[])
{
	...
	if (init(argc, argv))
	{
		...
		printf("         -f22 force speed to 22050 Hz (default)\n");
		printf("         -f44 force speed to 44100 Hz\n");	  	  	    
		printf("         -0 produces a TAPE 0 WAV file\n");				  
		printf("         -1 produces a TAPE 1 WAV file\n");		
		printf("         -2 produces a TAPE 2 WAV file\n");
		printf("         -3 produces a TAPE 3 WAV file (default)\n");
		printf("         -4 produces a TAPE 4 WAV file\n");
		printf("         -5 produces a TAPE 5 WAV file\n");	   	   
		printf("         -r6 force a TAPE 5 SQUARE WAV file with sampling freq=6KHz\n");
		printf("         -r9 force a TAPE 5 SQUARE WAV file with sampling freq =9KHz\n");  	   
		printf("         -q force SQUARE wave\n");		 	 
		exit(1);
	}
